...
two models are combined. For example, with alpha=0.9, the combined model means
 that the proportion of innovation models is large, while the proportion of 
 original models is small. It is a matter of proportion.
'''
...
def bilinear(adj,features):
    #bilinear matrices
    adj_, N_neig = preprocess_bilinear(adj)
    # Some preprocessing
    features= preprocess_features(features)
    return adj_, N_neig,features

adj_, N_neig,features = bilinear(adj,features)
...
def test_it(features,support,y_test,test_mask,placeholders):
    ...
    sess.close()
    r_time2 = time.time()
    print('early stop #epoch', epoch_early, 'val_loss=', "{:.5f}".format(vlss_early), 'val_acc=',
          "{:.4f}".format(vacc_early))
...
test_it(features,support,y_test,test_mask,placeholders)